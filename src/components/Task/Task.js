...
export default function Task( {task, onEditTask, onDeleteTask, onCompleteTask} ) {
    let [isChecked,setChecked] = React.useState(task.completed);
    const taskEditHandler = () => {
        const taskText = window.prompt('Введите название задачи');
        if (taskText) {
  ...
  return (
    <div className="tasks__items-row">
        <div className="checkbox">
          <input onChange={()=>completeTaskHandler(task.id)} id={`task-${task.id}`} type="checkbox" checked={isChecked}/>
